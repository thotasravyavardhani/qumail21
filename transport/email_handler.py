...
class EmailHandler:
    ...
    def __init__(self):
        ...
        # OAuth2 Provider Configuration (credentials loaded from secure environment variables)
        self.provider_config = {
            ...
            'yahoo': {
                ...
                'client_id': self.config.get('yahoo_client_id', ''),
                'client_secret': self.config.get('yahoo_client_secret', '')
            },
            'outlook': {
                ...
                'client_id': self.config.get('outlook_client_id', ''),
                'client_secret': self.config.get('outlook_client_secret', '')
            }
        }
        ...
        try:
            ...
            if user_profile:
                ...
                await self.email_db.store_user({
                    ...
                    'display_name': user_profile.name if hasattr(user_profile, 'name') else self.user_email,
                    'sae_id': getattr(user_profile, 'sae_id', ''),
                    'provider': 'qumail'
                })
    ...
    async def send_encrypted_email(self, to_address: str, encrypted_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if is_local_qumail_delivery:
                # DATABASE DELIVERY: Store email in shared database for multi-user access
                db_email_data = {
                    'email_id': email_data['email_id'],
                    'sender': self.user_email or "you@qumail.com",
                    'recipient': to_address,
                    'subject': encrypted_data.get('subject', 'Quantum Secured Message'),
                    'body': encrypted_data.get('body', ''),
                    'encrypted_payload': encrypted_data,
                    'security_level': encrypted_data.get('security_level', 'L2'),
                    'sent_at': email_data['sent_at']
                }
                
                # Store in database only (single source of truth)
                db_success = await self.email_db.store_email(db_email_data)
                
                if db_success:
                    logging.info(f"✅ DATABASE DELIVERY: Email stored in shared database for {to_address}")
            ...
            db_email_data = {
                ...
                'sender': self.user_email or "you@qumail.com",
                'recipient': to_address,
                'subject': encrypted_data.get('subject', 'Email'),
                'body': encrypted_data.get('body', ''),
                'encrypted_payload': encrypted_data,
            ...
            await self.email_db.store_email(db_email_data)
    ...
    async def fetch_email(self, email_id: str, recipient_email: str) -> Optional[Dict]:
        ...
        try:
            # Fetch from database
            inbox_emails = await self.email_db.get_inbox(recipient_email, limit=1000)
            sent_emails = await self.email_db.get_sent_emails(recipient_email, limit=1000)
            
            all_emails = inbox_emails + sent_emails
            
            for email_data in all_emails:
                if str(email_data.get('id')) == str(email_id) or str(email_data.get('email_id')) == str(email_id):
    ...
    async def get_email_list(self, folder: str = "INBOX", limit: int = 50) -> List[Dict]:
        ...
        try:
            ...
            if folder_key in ["INBOX", "Inbox"]:
                ...
                inbox_emails = await self.email_db.get_inbox(self.user_email, limit=1000)
                sent_emails = await self.email_db.get_sent_emails(self.user_email, limit=1000)
                all_emails = inbox_emails + sent_emails
                ...
                emails = sorted(secure_emails, key=lambda x: x.get('sent_at', ''), reverse=True)[:limit]
            else:
                # For other folders, try inbox as fallback
                emails = await self.email_db.get_inbox(self.user_email, limit)
